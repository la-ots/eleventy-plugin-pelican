...
module.exports = async function (eleventyConfig, options) {
  ...
  if (options.assets.core) {
    const pelicanPath = options.assets.pelicanPath
      ? options.assets.pelicanPath
      : path.join(__dirname, '../../../../../node_modules/@la-ots/pelican/')
    ...
    if (options.assets.core.css) {
      ...
      cssFiles = cssFiles.concat([
        path.join(pelicanPath, 'dist/css/pelican.min.css'),
        path.join(assetsPath, 'css/docs.scss'),
        path.join(assetsPath, 'css/prism-theme.css')
      ])
    }

    if (options.assets.core.js) {
      jsFiles = jsFiles.concat(options.assets.core.javascript)
    } else {
      jsFiles = jsFiles.concat([
        path.join(pelicanPath, 'dist/js/pelican.bundle.min.js'),
        path.join(assetsPath, 'js/docs.js')
      ])
    }
  }
