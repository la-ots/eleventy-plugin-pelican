...
module.exports = async function (eleventyConfig, options) {
  const relativeOutDir =
    (eleventyConfig.dir.output || "_site") + "/_pelican-assets";
  ...
  if (options.assets.sassLoadPaths) {
    ...
    sassLoadPaths = sassLoadPaths.filter(
      (item, pos) => sassLoadPaths.indexOf(item) === pos
    );
  }
  ...
  if (cssFiles.length > 0) {
    console.log(
      chalk.yellow(
        `[pelican] Compiling CSS to ${relativeOutDir}/${outFile}.css`
      )
    );
  ...
  if (jsFiles.length > 0) {
    console.log(
      chalk.yellow(
        `[pelican] Compiling Javascript to ${relativeOutDir}/${outFile}.js`
      )
    );
